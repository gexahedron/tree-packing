/*
 * File:   all_bipartite_graphs_packing.cpp
...
bool is_bipartite(const vector<pair<int, int>>& graph_edges) {
    int graph_n = get_vertex_count(graph_edges);
    vector<vector<int>> graph_children;
    vector<int> colors;

    for (int v = 0; v < graph_n; ++v) {
        graph_children.push_back({});
    }
    for (int v = 0; v < graph_n; ++v) {
        colors.push_back(-1);
    }
    ...
    while (colored) {
        ...
        for (const auto& edge: graph_edges) {
            ...
            if (colors[v1] != -1) {
                if (colors[v2] == -1) {
                    colors[v2] = 1 - colors[v1];
                    colored = true;
                } else if (colors[v2] == colors[v1]) {
                    return false;
                }
            } else if (colors[v2] != -1) {
                colors[v1] = 1 - colors[v2];
                colored = true;
            }
        }
    }
...
int main(int argc, char* argv[])
{
    ...
    for (int edge_cnt = 1; edge_cnt <= n - 1; edge_cnt++) {
        ...
        while (true) {
            ...
            // remove "nonbipartite" graphs
            if (!is_bipartite(graph_edges)) {
                continue;
            }
