...
short treeDatabase[20][30000][30];
static int totalAmounts[20] = {0};
...
void GenTree(int p, int s, int cL, int h, int l, int n, int f, int g) {
    ...
    if (p > n)
        if (f == 0) {
            ...
            updateL();
            //PrintIt();
            treeDatabase[N][totalAmounts[N]][0] = N;
            for (size_t i = 1; i <= static_cast<size_t>(N); ++i)
                treeDatabase[N][totalAmounts[N]][i] = par[i] - 1;
            ++totalAmounts[N];
        }
    else {
        if (cL == 0) {
